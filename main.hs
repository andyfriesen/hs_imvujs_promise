...
mkPromise = do
    ...
    promise <- newPromise "mkPromise" $ writeIORef r
...
mkAcceptor :: IO (IORef [result], result -> IO (Promise result err))
mkAcceptor = mkCallback acceptedPromise

mkRejector :: IO (IORef [err], err -> IO (Promise result err))
mkRejector = mkCallback rejectedPromise
...
main = defaultMain $ testGroup "Tests"
    ...
    --         p `then2` (\value -> rejectedPromise value) >>=
    --          (`catch` (\error -> writeIORef result (Just error) >> rejectedPromise ()))
